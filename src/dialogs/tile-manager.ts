...
export class TileManagerDialog extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  async _prepareContext(_options: any): Promise<any> {
    ...
    // module manifest at world startup and only refreshes on full world reload
    // — not on hot-reload of the dist bundle. As a result the footer would show
    // a stale version after rebuilding while the build number updated
    // immediately, making them look out of sync. Baking both in keeps them in
    // lockstep.
    const version = __MODULE_VERSION__ || '1.0.0';
