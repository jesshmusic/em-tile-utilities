 ...
 *     'all' | 'any' | 'none' (actions.js, `values.type`), and it aggregates across the
 *     *entities* the action resolved to, not across conditions. We always target one
 *     specific tile, so 'all' is correct.
 *   - The comparison lives in `checkvariable.data.value`, which MATT runs through
 *     `MonksActiveTiles.getValue(..., { prop, operation: 'compare' })`
 *     (monks-active-tiles.js:324). In compare mode it builds a JS expression
 *     `<prop> <value>` and evals it, passing the value through untouched when it starts
 *     with `==`, `>`, `<` or `!=`, and prefixing a bare value with `== `.
 *
 * So every operator in ConditionOperator is expressible: `>=` and `<=` survive because
 * MATT's guard tests `startsWith('>')` / `startsWith('<')`.
 */
 ...
 *
 * Operates on the resolved `{ condition, tile, value }` entries built above.
 */
