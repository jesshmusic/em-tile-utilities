...
/** Legacy flag scope: tiles created before v2.0.2 stored the key inside Monk's own flags. */
...
export async function createCombatTrapTile(
  ...
  try {
    ...
    if (config.maxTriggers > 0) {
      ...
      // `{{add variable.X 1}}`. Neither helper exists: MATT evaluates action
      // values against the global Handlebars instance, where the only helpers
      // registered are Foundry's (eq/ne/lt/gt/lte/gte/not/and/or/concat/…),
      // MATT's `selectGroups` and whatever the system adds — no `default`, no
      // `add`. Handlebars' `helperMissing` throws when an unknown helper is
      // called *with arguments*, so both actions blew up and trigger limiting
      // never worked.
      //
      // MATT's native increment does both jobs in one action: `setvariable.fn`
      // seeds an unset variable to 0 when the value starts with "+"/"-" and
      // getValue then evaluates `<current> + 1`.
      ...
      // `checkvariable.data.type` only accepts all/any/none and the old 'gte'
      // matched none of them, so the filter always fell through to the fail
      // anchor. `getValue(..., {operation:'compare'})` passes a leading ">"
      // through untouched and evals `<count> > <max>`.
      //
    ...
    // Tag the combat trap tile using Tagger if available. The user's custom tags
    // were previously dropped on the floor here.
    ...
    // Deliberately not re-thrown: the only caller, trap-dialog.ts, doesn't guard
    // this call and an escaping rejection would strand its placement dialog.
    // Returning null lets callers detect failure.
    return null;
  }
}
